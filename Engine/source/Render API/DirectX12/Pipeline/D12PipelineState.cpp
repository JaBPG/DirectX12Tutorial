...
namespace Engine {
	...
	void D12PipelineState::Initialize(ID3D12Device* pDevice)
	{
		...
		mShaders[0].Initialize(L"shaders/base/VS.hlsl", HLSLShader::ShaderType::VERTEX);
		mShaders[1].Initialize(L"shaders/base/PS.hlsl", HLSLShader::ShaderType::PIXEL);
		...
		desc.RasterizerState.CullMode = D3D12_CULL_MODE_FRONT; //might do none
	...
	void D12PipelineState::InitializeDeferred(ID3D12Device* pDevice)
	{
		mRootSignature.Initialize(pDevice);
		mShaders[0].Initialize(L"shaders/deferred/VS.hlsl", HLSLShader::ShaderType::VERTEX);
		mShaders[1].Initialize(L"shaders/deferred/PS.hlsl", HLSLShader::ShaderType::PIXEL);
	...
	void D12PipelineState::InitializePixelProces(ID3D12Device* pDevice, std::wstring pixelshaderpath)
	{
		mRootSignature.Initialize(pDevice);

		mShaders[0].Initialize(L"shaders/postfx/VS.hlsl", HLSLShader::ShaderType::VERTEX);
		mShaders[1].Initialize(pixelshaderpath.c_str(), HLSLShader::ShaderType::PIXEL);
	...
	void D12PipelineState::InitializeShadowMapping(ID3D12Device* pDevice)
	{
		mRootSignature.Initialize(pDevice);
		mShaders[0].Initialize(L"shaders/shadowmap/VS.hlsl", HLSLShader::ShaderType::VERTEX);
		//mShaders[1].Initialize(L"shaders/base/PS.hlsl", HLSLShader::ShaderType::PIXEL);
